...
function isVowel(key){
    if(key.length === 1 && key.match(/[aeiou]/i)){return true;}
    else{return false;
    }
}

function isConsonant(key){
    if(key.length === 1 && key.match(/[bcdfghjklmnpqrstvwxyz]/i)){return true;}
    else{return false;
    }
}
function addLetter(letter){
    ...
    state.grid[state.currentRow][state.currentCol] = wanakana.toHiragana(letter);
    state.currentCol++;
    }
...
function removeLetter(){
    ...
    if(state.grid[state.currentRow][state.currentCol] !== '' && state.currentCol !== 5){
